        ...
        <div className="space-y-4">
          {useCases.map((useCase, index) => (
            ...
            >
              ...
              <div>
                <span className="text-xl font-black uppercase mr-2">{useCase.title}</span>
                <span className="text-lg font-medium text-gray-800 hidden md:inline">— {useCase.text}</span>
                <p className="text-lg font-medium text-gray-800 md:hidden mt-1">{useCase.text}</p>
              </div>
            </div>
          ))}
        </div>
